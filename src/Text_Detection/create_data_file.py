...
def createdata(x, y):
    a=0
    for filename in glob.glob(dataset_path + '*.png'):
        list_helper = 0
        tmp_image = cv2.imread(filename)
        for resized in pyramid(tmp_image):
            # loop over the sliding window for each layer of the pyramid
            pyramid_counter=0
            for (x, y, window) in sliding_windows(resized, stepSize=12, windowSize=(winW, winH), startX=0, startY=0):
...
def create_dataset_matrix(T, featureSize):
    ...
    for filename in glob.glob(dataset_path + '*.png'):
        ...
        tmp_array = np.array(img).reshape(1, featureSize)
        dataset_matrix[startIndex, :] = tmp_array
        startIndex += 1
        if startIndex % 5000 == 0:
            print("%{}. Completed..".format(startIndex/500))
    print("Dataset Matrix Succesfully Created.. Size Of: {}x{}".format(T,featureSize))
    return dataset_matrix


def create_matfile():
    ds = create_dataset_matrix(50000,6144)
    convert_type = np.array(ds,dtype='uint8')
    # for i in range(50):
    #     ds = shuffle(ds)
    #     print("Shuffled.. {}.Th Time".format(i+1))
    sio.savemat('negTrain.mat', mdict={'negdataset': convert_type})

# GOTTA FIX
def determine_labels():
    mat = sio.loadmat('tdtrain_wl.mat')
    ds = mat.get('dataset')
    print(ds[1800:1900,0])
    for x in range(1999, 3001):
        if x % 250 == 0 and x != 0:
            print("Saving Changes...")
            sio.savemat('tdtrain_wl.mat', mdict={'dataset': ds})
            ...
            if keep == 'y':
                ...
                exit(0)
        tmp = ds[x, 1:].reshape(32, 64, 3)
        img = Image.fromarray(tmp)
        img.show()
        os.system('xdotool search "mirac@outlier ~/Dev/" windowactivate ')
